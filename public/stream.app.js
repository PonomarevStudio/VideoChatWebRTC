...
async function connect() {
    ...
    let room = query.has('room') ? query.get('room') : null;
    if (!room || room === lastRoom) room = prompt('Введеите id комнаты');
    lastRoom = room;
    console.log('Connect')
    ...
    return await joinRoomById(room, 'Streamer');
}
...
function startTimer() {
    ...
    return timer = setInterval(() => time <= 0 ? reConnect() : true
